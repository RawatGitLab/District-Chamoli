...
export default function Sidebar({
  ...
  const otherLayers = useMemo(() => {
    return layers.filter(layer => {
      ...
      if (isPolice || isHealth || isRiver || isEducation || isAdministrative || isPolling || isIrrigation || isMinority || isVillage || isEmployment || isTown || isTourist || isUrban) {
        return false;
      }
      ...
      return !isAdministrative && !isEducation && !isHealth && !isPolice && !isRiver && !isPolling && !isIrrigation && !isMinority && !isVillage && !isEmployment && !isTown && !isTourist && !isUrban;
    });
  }, [layers]);
      ...
      <div className="p-3 border-t border-slate-200 bg-white">
        <div className="bg-slate-50 border border-slate-100 rounded-md p-2 flex flex-col gap-1 text-[11px] text-slate-500 font-medium">
          ...
          <div className="flex justify-between">
            ...
            <span className="font-mono text-slate-700 font-bold">057</span>
          </div>
        </div>
      </div>
    </aside>
