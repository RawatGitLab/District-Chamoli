...
export default function MapComponent({
  ...
  useEffect(() => {
    ...
    // Center on Chamoli, Uttarakhand, India (Chamoli: 30.45, 79.41 - shifted south to move shapes upward on the screen)
  ...
  useEffect(() => {
    ...
    if (features.length > 0 && isInitialLoadRef.current) {
      ...
      const tryFitBounds = () => {
        try {
          // Calculate bounds directly from the loaded GIS features
          ...
          if (hasValidCoords) {
            ...
            if (isBoundsValid(bounds) && size.x > 0 && size.y > 0) {
              const zoom = map.getBoundsZoom(bounds, false, [80, 80]);
              const center = bounds.getCenter();
              const shiftedCenter = L.latLng(center.lat - 0.05, center.lng - 0.18);
              map.setView(shiftedCenter, Math.min(zoom, 13));
  ...
  // Map Controls: Pan to Chamoli bounds safely
  const handleZoomToDistrict = () => {
      ...
      if (activeGeoJsons.length > 0) {
        ...
        if (isBoundsValid(bounds) && size.x > 0 && size.y > 0) {
          ...
          if (isFinite(calculatedZoom) && !isNaN(calculatedZoom)) {
            const center = bounds.getCenter();
            const shiftedCenter = L.latLng(center.lat - 0.05, center.lng - 0.18);
            map.setView(shiftedCenter, Math.min(calculatedZoom, 13), { animate: true });
            return;
          }
        }
      }
    ...
    // Fallback static center of Chamoli
    map.setView([30.45, 79.41], 9, { animate: true });
  };
